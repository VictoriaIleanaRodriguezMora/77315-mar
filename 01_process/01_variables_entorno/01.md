...
# ¿Qué es process? - variable global
...
⚠️ Todas las variables dentro de `process.env` son strings.
Si necesitamos números o booleanos debemos convertirlos manualmente.
Ejemplo:
```js
# Si necesitaramos el puerto en Nro
const port = Number(process.env.PORT);
```

❗Modificar `process.env` en tiempo de ejecución afecta solo al proceso actual.

# `process.stdin`
Es el canal por donde un programa recibe datos mientras se está ejecutando.
- cuando escribís en la terminal
- cuando pegás texto
- cuando otro programa le manda datos
- cuando un test automático le pasa inputs
Node está "escuchando"
...
❗Un sólo guión (-), lleva una sóla letra
```js
commander.option("-p <port>", "port server", 8080);
```

Para configurar un comando de 2 letras
```js
commander.option("--p <port>", "port server", 8080);
```
...
```js
console.log(commander.args); // los argumentos sueltos que le envío, porque puedo pasar argumentos sin flags que tenga configurado. sigo teniendo acceso a ellos en un array
```
